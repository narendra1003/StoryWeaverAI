...
# Main app logic
def main():
    # Get user name - no submit button needed
    st.session_state.user_name = st.text_input("Please share your name:", key="name_input", value=st.session_state.user_name).strip()
    
    # Always show these buttons
    col1, col2 = st.columns([1, 1])
    with col1:
        weave_story = st.button("Weave A Story")
    with col2:
        quit_session = st.button("Quit Session")
    ...
    # Check if we should show story controls
    if st.session_state.user_name and st.session_state.show_story_controls:
        ...
        if st.session_state.intro_message:
            ...
            # Generate introduction if we haven't yet
